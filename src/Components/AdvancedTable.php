...
final class AdvancedTable extends Component
{
    ...
    {
        if ((bool) $this->option('loading', false)) return $this->skeleton($columns, $selectable, $actions);
        if ($this->rows === []) return '<tr class="stl-advanced-table__empty"><td colspan="'.$columnCount.'">'.$this->empty().'</td></tr>';
    ...
    {
        $title = $this->option('title'); $description = $this->option('description');
    ...
    private function actions(array|object $row): string
    {
        ...
        foreach ($this->actions as $action) {
            if (!$this->allowed($action, $row)) continue;
    ...
    private function value(array|object $row, string $path): mixed { $value = $row; foreach (explode('.', $path) as $key) { if (is_array($value)) $value = $value[$key] ?? null; elseif (is_object($value)) $value = $value->{$key} ?? null; else return null; } return $value; }
    private function content(mixed $value): string { return $value instanceof Renderable ? $value->render() : Html::escape((string) $value); }
    private function renderMany(mixed $values): string { return is_array($values) ? implode('', array_map(fn ($value) => $this->content($value), $values)) : $this->content($values); }
    private function option(string $key, mixed $default = null): mixed { return $this->options[$key] ?? $default; }
    private function hasVisibleActions(): bool { foreach ($this->actions as $action) if (($action['permission'] ?? true) !== false && ($action['visible'] ?? true) !== false) return true; return false; }
}
