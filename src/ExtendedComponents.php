...
final class ExtendedComponents
{
    ...
    {
        $id = 'stl-list-'.substr(md5($name), 0, 8);
        $out = '';
        foreach ($options as $option) {
            $out .= '<option value="'.Html::escape($option).'"></option>';
        }
    ...
    private static function breadcrumbs(array $items, array $attributes): string
    {
        ...
        foreach ($items as $i => $item) {
            $label = Html::escape(is_array($item) ? $item['label'] : $item);
            $href = is_array($item) ? ($item['href'] ?? null) : null;
            $out .= '<li>'.($href && $i !== $last ? '<a href="'.Html::escape($href).'">'.$label.'</a>' : '<span aria-current="page">'.$label.'</span>').'</li>';
        }
    ...
    private static function calendar(string $month, array $attributes): string
    {
        ...
        $offset = (int) $date->format('N');
        $cells = str_repeat('<span></span>', $offset - 1);
        for ($day = 1; $day <= $days; $day++) {
            $cells .= '<button type="button"'.($day === (int) date('j') && $date->format('Y-m') === date('Y-m') ? ' aria-current="date"' : '').'>'.$day.'</button>';
        }
    ...
    private static function carousel(array $slides, array $attributes): string
    {
        ...
        foreach ($slides as $i => $slide) {
            $out .= '<div class="stl-carousel__slide"'.($i ? ' hidden' : '').' data-stl-slide><strong>'.Html::escape(is_array($slide) ? $slide['title'] : $slide).'</strong>'.(is_array($slide) ? '<p>'.Html::escape($slide['text'] ?? '').'</p>' : '').'</div>';
        }
    ...
    private static function chart(array $values, string $label, array $attributes): string
    {
        ...
        foreach ($values as $name => $value) {
            $bars .= '<span style="--value:'.((float) $value / $max * 100).'%"><i></i><small>'.Html::escape($name).'</small><b>'.Html::escape($value).'</b></span>';
        }
    ...
    private static function command(array $items, string $placeholder, array $attributes): string
    {
        ...
        foreach ($items as $item) {
            $out .= '<button type="button" data-stl-command-item>'.Html::escape($item).'</button>';
        }
    ...
    private static function menu(array $items, string $class, string $trigger): string
    {
        ...
        foreach ($items as $label => $href) {
            $out .= '<a href="'.Html::escape(is_string($label) ? $href : '#').'">'.Html::escape(is_string($label) ? $label : $href).'</a>';
        }
    ...
    {
        $out = '';
        foreach ($columns as $title => $cards) {
            $items = '';
            foreach ($cards as $card) {
                $items .= '<article draggable="true">'.Html::escape($card).'</article>';
            } $out .= '<section><header><strong>'.Html::escape($title).'</strong><small>'.count($cards).'</small></header>'.$items.'</section>';
        }
...
return '<label class="stl-field"><span class="stl-field__label">'.Html::escape($label).'</span><select class="stl-select" name="'.Html::escape($name).'"'.Html::attributes($attributes).'>'.$out.'</select></label>';
    ...
    private static function table(array $columns, array $rows, array $attributes): string
    {
        ...
        foreach ($columns as $column) {
            $head .= '<th scope="col">'.Html::escape($column).'</th>';
        } $body = '';
        foreach ($rows as $row) {
            $body .= '<tr>';
            foreach ($row as $cell) {
                $body .= '<td>'.self::content($cell).'</td>';
            } $body .= '</tr>';
        }
...
return '<div class="stl-table-wrap"><table class="stl-table"'.Html::attributes($attributes).'><thead><tr>'.$head.'</tr></thead><tbody>'.$body.'</tbody></table></div>';
    ...
    {
        $buttons = $panels = '';
        $active = $active ?: (string) array_key_first($tabs);
        foreach ($tabs as $label => $content) {
            $id = 'stl-tab-'.substr(md5($label), 0, 8);
            $selected = $label === $active;
            $buttons .= '<button id="'.$id.'-button" role="tab" aria-controls="'.$id.'" aria-selected="'.($selected ? 'true' : 'false').'">'.Html::escape($label).'</button>';
